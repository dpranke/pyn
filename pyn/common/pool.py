...
def _stub_post(context_after_pre):
    return context_after_pre


def make_pool(host, jobs, callback, context=None, pre_fn=None, post_fn=None):
    pre_fn = pre_fn or _stub_pre
    post_fn = post_fn or _stub_post
...
class _ProcessPool(object):
    ...
    def get(self, block=True):
        msg_type, resp = self.responses.get(block=block)
